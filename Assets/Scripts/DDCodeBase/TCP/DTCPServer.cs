...
public class DTCPServer : MonoBehaviour
{
    //服务器端套接字
    ...
    private void AcceptCb(IAsyncResult ar)
    {
        ...
        {
            //获取接入的socket
            Socket socket = TCPServer.EndAccept(ar);
    ...
    public void SendMessage(byte[] sendBytes)
    {
        ...
        {
            //获取远程终结点的IP和端口信息
            IPEndPoint ipe = (IPEndPoint)TCPServer.RemoteEndPoint;
            TCPServer.Send(sendBytes, sendBytes.Length, 0);
        }
    }
    ...
    private void ReceiveMessage(Socket socketClient)
    {
        ...
        while (HeadLength > 0)
        {
            ...
            {
                iBytesHead = TCPServer.Receive(recvBytes1, recvBytes1.Length, 0);
            }
            else
            {
                iBytesHead = TCPServer.Receive(recvBytes1, HeadLength, 0);
            }
            //将接收到的字节数保存
        ...
        while (BodyLength > 0)
        {
            ...
            {
                iBytesBody = TCPServer.Receive(recvBytes2, recvBytes2.Length, 0);
            }
            else
            {
                iBytesBody = TCPServer.Receive(recvBytes2, BodyLength, 0);
            }
            //将接收到的字节数保存
