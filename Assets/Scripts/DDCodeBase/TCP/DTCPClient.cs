...
public class DTCPClient
{
    ...
    public void Connetion(string host, int port)
    {
        ...
        //开始异步接受数据
        TCPClient.BeginReceive(readBuff, 0, readBuffSize, SocketFlags.None, ReceiveCb, null);
    }
    ...
    public byte[] BuildDataPackage(int Crccode, long sessionid, int command, int subcommand, int encrypt, string[] MessageBody)
    {
        //消息校验码默认值为0x99FF
        Crccode = 65433;
    ...
    public void SendMessage(byte[] sendBytes)
    {
        ...
        {
            //获取远程终结点的IP和端口信息
            IPEndPoint ipe = (IPEndPoint)TCPClient.RemoteEndPoint;
            TCPClient.Send(sendBytes, sendBytes.Length, 0);
        }
    }
    ...
    private void ReceiveMessage()
    {
        while (true)
        {
            ...
            while (HeadLength > 0)
            {
                ...
                {
                    iBytesHead = TCPClient.Receive(recvBytes1, recvBytes1.Length, 0);
                }
                else
                {
                    iBytesHead = TCPClient.Receive(recvBytes1, HeadLength, 0);
                }
                //将接收到的字节数保存
            ...
            while (BodyLength > 0)
            {
                ...
                {
                    iBytesBody = TCPClient.Receive(recvBytes2, recvBytes2.Length, 0);
                }
                else
                {
                    iBytesBody = TCPClient.Receive(recvBytes2, BodyLength, 0);
                }
                //将接收到的字节数保存
